...
void ColorConverterFilter<inputImageT, outputImageT>::rgbToHsv()
{
    ...
    while (!inputIt.IsAtEnd() )
    {
            ...
            pixelFloat = static_cast<outputPixelT>(inputIt.Get());
...
void ColorConverterFilter<inputImageT, outputImageT>::rgbToHsl()
{
    ...
    if constexpr (std::is_floating_point<outputPixelCompT>::value)
    {
        ...
        while (!inputIt.IsAtEnd() )
        {
            ...
            rgb = static_cast<outputPixelT>(inputIt.Get());
            //rgb = static_cast<outputPixelT>(inputIt.Get())/255.0;

            rgb.SetRed(rgb.GetRed()/255.0);
            rgb.SetGreen(rgb.GetGreen()/255.0);
            rgb.SetBlue(rgb.GetBlue()/255.0);
...
void ColorConverterFilter<inputImageT, outputImageT>::rgbToXyz()
{
    ...
    while (!inputIt.IsAtEnd() )
    {
        ...
        rgb = sRGBInverseCompanding( (inputIt.Get()) );
        //rgb[0] /= 255.0;
        //rgb[1] /= 255.0;
        //rgb[2] /= 255.0;
        ...
        /* std::cout<<inputIt.Get()<<" -> "<<XYZ<<std::endl; */
...
ColorConverterFilter<inputImageT, outputImageT>::sRGBInverseCompanding(const outputPixelT& rgbPixel)
{
    ...
    for (unsigned i=0 ; i < 3 ; ++i)
    {
        //std::cout<<"SSS>"<<rgbPixel[i]/255.0<<std::endl;

        v[i] = (rgbPixel[i]/255.0 <= 0.04045) ? (rgbPixel[i]/255.0) / 12.92 : std::pow( ( (rgbPixel[i]/255.0) + 0.055) / 1.055, 2.4);
